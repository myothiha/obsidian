Ref:
https://www.mckinsey.com/capabilities/risk-and-resilience/our-insights/banking-on-gen-ai-in-the-credit-business-the-route-to-value-creation
https://www.mckinsey.com/industries/financial-services/our-insights/banking-matters/banking-trends-snapshot-how-banks-can-catch-up-to-fintechs-on-ai
https://www.mckinsey.com/industries/financial-services/our-insights/extracting-value-from-ai-in-banking-rewiring-the-enterprise

# Survey Result

### Survey on how Banks are deploying AI agents
- Majority focus on concision (get small chunk of insights from big data.)

### Important Factors
- Productivity (47)
- Business Needs (44)
- Regulatory Compliance (25)

Note: regional banks are leading in AI deployment compared to Mega banks.

### Gaps
No banks have done
- Full deployment on synthesizing information for credit decisioning
- Content generation use cases such as drafting of credit memos and data assessment are also among the most piloted.

Risks
- Insufficient accuracy
- Lack of articulation (explanation?)
- hallucination